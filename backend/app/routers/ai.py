...
def get_prompts(current_user: Optional[User] = Depends(get_current_user)):
    ...
    try:
        ...
        if user_id is None:
            ...
            # 多用户模式：返回“当前用户 + 系统级(user_id IS NULL)”模板
            # 去重策略：若用户已存在同名模板，则隐藏系统同名模板（避免列表重复）
            cursor.execute("""
                WITH merged AS (
                    SELECT id, name, system_prompt, user_prompt, is_default, created_at, updated_at, 0 AS source_priority
                    FROM ai_prompts
                    WHERE user_id = ?

                    UNION ALL

                    SELECT s.id, s.name, s.system_prompt, s.user_prompt, s.is_default, s.created_at, s.updated_at, 1 AS source_priority
                    FROM ai_prompts s
                    WHERE s.user_id IS NULL
                      AND NOT EXISTS (
                          SELECT 1 FROM ai_prompts u
                          WHERE u.user_id = ? AND u.name = s.name
                      )
                )
                SELECT id, name, system_prompt, user_prompt, is_default, created_at, updated_at
                FROM merged
                ORDER BY is_default DESC, source_priority ASC, id ASC
            """, (user_id, user_id))
