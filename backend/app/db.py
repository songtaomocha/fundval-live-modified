...
def init_db():
    ...
    default_settings = [
        ...
        ('MODEL_CALIBRATION_LOOKBACK_DAYS', '20', 0),
        ('MODEL_TICK_MAX_JUMP_PCT', '2.8', 0),
        ('MODEL_TICK_MAX_SLOPE_PCT_PER_MIN', '0.9', 0),
        ('MODEL_EVAL_LOOKBACK_DAYS', '20', 0),
    ]
...
- suggestions: 投资建议列表（2-4条）"""
    ...
    # Migration 10: Remove built-in default AI prompts globally
    if current_version < 10:
        logger.info("Running migration 10: Remove built-in default AI prompts")
        cursor.execute("""
            DELETE FROM ai_prompts
            WHERE name IN ('Linus 风格', '温和风格')
        """)
        logger.info(f"Migration 10 deleted {cursor.rowcount} built-in prompts")
        cursor.execute("INSERT OR IGNORE INTO schema_version (version) VALUES (10)")

    # Migration 11: valuation model calibration/evaluation tables
    if current_version < 11:
        logger.info("Running migration 11: add fund valuation model tables")

        cursor.execute("""
            CREATE TABLE IF NOT EXISTS fund_model_calibration (
                code TEXT PRIMARY KEY,
        ...
        cursor.execute("""
            CREATE TABLE IF NOT EXISTS fund_model_eval_daily (
                code TEXT NOT NULL,
    ...
    cursor.execute("CREATE INDEX IF NOT EXISTS idx_fund_history_code_date ON fund_history(code, date)")
    cursor.execute("CREATE INDEX IF NOT EXISTS idx_intraday_code_date_time ON fund_intraday_snapshots(fund_code, date, time)")
