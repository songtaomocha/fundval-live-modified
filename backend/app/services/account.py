...
def get_all_positions(account_id: int, user_id: Optional[int] = None) -> Dict[str, Any]:
    ...
    # Batch query 2: Get latest NAV dates/values for all codes
    from datetime import datetime
    today_str = datetime.now().strftime("%Y-%m-%d")
    cursor_batch.execute(f"""
        SELECT fh.code, fh.date as latest_date, fh.nav
        ...
        JOIN (
            ...
            GROUP BY code
        ) t ON fh.code = t.code AND fh.date = t.latest_date
    """, codes)
    nav_rows = cursor_batch.fetchall()
    nav_date_map = {row["code"]: row["latest_date"] for row in nav_rows}
    nav_value_map = {row["code"]: float(row["nav"]) for row in nav_rows}
    conn_batch.close()
    ...
    with ThreadPoolExecutor(max_workers=10) as executor:
        ...
        try:
            for future in as_completed(future_to_code, timeout=8):
    ...
    # Fill any missing positions quickly with latest NAV fallback (avoid first-load long blank wait)
    existing_codes = {p.get("code") for p in positions}
    for code, row in position_map.items():
        if code in existing_codes:
            continue

        fund_info = fund_info_map.get(code, {})
        name = fund_info.get("name") or code
        fund_type = fund_info.get("type") or get_fund_type(code, name)
        nav = float(nav_value_map.get(code, 0.0))
        shares = float(row["shares"])
        cost = float(row["cost"])
        cost_basis = cost * shares
        nav_market_value = nav * shares
        ...
        positions.append({
            ...
            "shares": shares,
            "nav": nav,
            "nav_date": nav_date_map.get(code, "--"),
            "nav_updated_today": nav_date_map.get(code) == today_str if nav_date_map.get(code) else False,
            "estimate": nav,
            "est_rate": 0.0,
            "is_est_valid": False,
            "cost_basis": round(cost_basis, 2),
            "nav_market_value": round(nav_market_value, 2),
            "est_market_value": round(nav_market_value, 2),
            "accumulated_income": round(accumulated_income, 2),
            "accumulated_return_rate": round(accumulated_return_rate, 2),
            "day_income": 0.0,
            "total_income": round(accumulated_income, 2),
            "total_return_rate": round(accumulated_return_rate, 2),
            "update_time": "--"
        })

        total_market_value += nav_market_value
        total_cost += cost_basis
