...
# /api/account/positions short cache + stale fallback + in-flight de-dup
_ACCOUNT_POS_CACHE = {}
_ACCOUNT_POS_INFLIGHT = {}
_ACCOUNT_POS_LOCK = threading.Lock()
_ACCOUNT_POS_CACHE_TTL = 8
_ACCOUNT_POS_STALE_TTL = 180
...
def get_aggregate_positions(current_user: Optional[User] = Depends(get_current_user)):
    ...
    try:
        ...
        # 批量预取基金元数据与最新净值日期，避免线程内 N+1 查询
        code_list = list(position_map.keys())
        fund_meta_map = {}
        latest_nav_date_map = {}

        if code_list:
            conn_prefetch = get_db_connection()
            try:
                cursor_prefetch = conn_prefetch.cursor()
                placeholders_codes = ",".join("?" * len(code_list))

                cursor_prefetch.execute(
                    f"SELECT code, name, type FROM funds WHERE code IN ({placeholders_codes})",
                    code_list,
                )
                for r in cursor_prefetch.fetchall():
                    fund_meta_map[r["code"]] = {
                        "name": r["name"],
                        "type": r["type"],
                    }

                cursor_prefetch.execute(
                    f"SELECT code, MAX(date) AS latest_date FROM fund_history WHERE code IN ({placeholders_codes}) GROUP BY code",
                    code_list,
                )
                for r in cursor_prefetch.fetchall():
                    latest_nav_date_map[r["code"]] = r["latest_date"]
            finally:
                conn_prefetch.close()
        ...
        with ThreadPoolExecutor(max_workers=10) as executor:
            ...
            for future in as_completed(future_to_code):
                ...
                try:
                    ...
                    if not fund_type:
                        fund_type = (fund_meta_map.get(code) or {}).get("type") or get_fund_type(code, name)

                    latest_nav_date = latest_nav_date_map.get(code)
                    nav_updated_today = bool(latest_nav_date and latest_nav_date == today_str)
...
def get_positions(
    ...
    user_id = get_user_id_for_query(current_user)
    cache_key = f"{user_id or 0}:{account_id}"
    now = time.time()

    cached = _ACCOUNT_POS_CACHE.get(cache_key)
    if cached and now - cached["ts"] <= _ACCOUNT_POS_CACHE_TTL:
        return cached["data"]

    owner = False
    with _ACCOUNT_POS_LOCK:
        inflight = _ACCOUNT_POS_INFLIGHT.get(cache_key)
        if inflight is None:
            inflight = threading.Event()
            _ACCOUNT_POS_INFLIGHT[cache_key] = inflight
            owner = True

    if not owner:
        inflight.wait(timeout=8)
        cached = _ACCOUNT_POS_CACHE.get(cache_key)
    ...
    try:
        ...
        if owner:
            with _ACCOUNT_POS_LOCK:
                event = _ACCOUNT_POS_INFLIGHT.pop(cache_key, None)
                if event:
                    event.set()
