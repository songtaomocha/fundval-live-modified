...
from ..services.fund import get_combined_valuation, run_daily_model_evaluation
...
def collect_intraday_snapshots():
    ...
    Only runs on trading days between 09:00-15:00.
    ...
    # 2. Check if within collection window (09:00-15:00)
    current_time = now_cst.strftime("%H:%M")
    if current_time < "09:00" or current_time > "15:00":
        return
    ...
    def _extract_watchlist_codes(raw_value):
        """
        user_watchlist may be:
        - ["001194", "010826"]
        - [{"id": "001194", ...}, {"id": "010826", ...}]
        Return normalized fund code list.
        """
        result = []
        if not raw_value:
            return result
    ...
    if watchlist_row and watchlist_row["value"]:
        codes.extend(_extract_watchlist_codes(watchlist_row["value"]))

    # Multi-user mode watchlist
    cursor.execute("SELECT value FROM user_settings WHERE key = 'user_watchlist'")
    for row in cursor.fetchall():
        if row["value"]:
            codes.extend(_extract_watchlist_codes(row["value"]))

    # Normalize + de-duplicate; avoid unhashable items
    codes = sorted({str(c).strip() for c in codes if c and str(c).strip()})
        ...
        try:
            ...
            if data and data.get("estimate"):
                ...
                if fallback_row and fallback_row["estimate"] is not None:
                    ...
                    skipped += 1
                    logger.warning(f"Skipped {code}: no estimate data and no fallback snapshot (data={data})")
    ...
    def _run():
        ...
        while True:
            try:
                ...
                # Daily model evaluation (once per day after 20:00 CST)
                if now_cst.hour >= 20 and last_eval_date != today_str:
                    eval_result = run_daily_model_evaluation(trade_date=today_str)
                    logger.info(f"Daily model evaluation finished: {eval_result}")
